...
#        if len(fileout) > 0: #output template
    ...
    def _on_add_folder(self):
        ...
        self.tree.see(focus)
        self.status['text'] = LOCALIZED_TEXT[lang]["Renaming files..."]
        self.update_idletasks()
        self._rename_children_of(self.project_id)
    ...
    def _add_tree(self, the_focus, adir_path, noTop=False):
        ...
        lang = self.ddnGuiLanguage.get()
        qcommand.put('ADD_TREE', self.tree)
        while True:
            areport, self.tree = qreport.get()
            self.status['text'] = areport
            self.update_idletasks()
            qreport.end_task()
        return
